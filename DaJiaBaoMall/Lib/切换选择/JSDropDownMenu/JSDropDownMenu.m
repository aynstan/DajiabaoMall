...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (i==2) {
            indicator = [self createIndicatorWithColor:[UIColor clearColor] andPosition:CGPointMake(titlePosition.x + title.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
        }else{
            indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake(titlePosition.x + title.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
        }
    ...
    _bottomShadow.backgroundColor = RGB(231, 231, 232);
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height inView:(UIView *)view{
    self.inView=view;
    CGSize screenSize = CGSizeMake(view.frame.size.width, view.frame.size.height);
    ...
    if (self) {
        ...
        _collectionView.tintColor = [UIColor redColor];
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(self.origin.x, self.origin.y+self.frame.size.height, self.inView.frame.size.width, self.inView.frame.size.height- self.origin.y-self.frame.size.height)];
        _backGroundView.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.0];
        _backGroundView.opaque = NO;
        UIGestureRecognizer *gesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(backgroundTapped:)];
        [_backGroundView addGestureRecognizer:gesture];
        [self.inView addSubview:_backGroundView];
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(0, 0, self.frame.size.width/self.numOfMenu, self.frame.size.height-0.75);
...
- (CAShapeLayer *)createSeparatorLineWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    [path addLineToPoint:CGPointMake(160, 34)];
    layer.path = path.CGPath;
    layer.lineWidth = 0.5;
...
- (void)animateIndicator:(CAShapeLayer *)indicator Forward:(BOOL)forward complete:(void(^)())complete {
    ...
    if (indicator==_indicators[2]) {
        indicator.fillColor = [UIColor clearColor].CGColor;
    }else{
        indicator.fillColor = forward ? _leftTableView.tintColor.CGColor : _indicatorColor.CGColor;
    }
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (leftTableView) {
            leftTableView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height-0.25, self.frame.size.width*ratio, 0);
            [self.superview addSubview:leftTableView];
            leftTableViewHeight = ([leftTableView numberOfRowsInSection:0] > (self.inView.frame.size.height-self.origin.y-self.frame.size.height)/leftTableView.rowHeight) ? (self.inView.frame.size.height-self.origin.y-self.frame.size.height) : ([leftTableView numberOfRowsInSection:0] * leftTableView.rowHeight);
        }
        if([self.dataSource haveRightTableViewInColumn:_currentSelectedMenudIndex]){
            if (rightTableView) {
                rightTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height-0.25, self.frame.size.width*(1-ratio), 0);
                [self.superview addSubview:rightTableView];
                 rightTableViewHeight = ([rightTableView numberOfRowsInSection:0] >  (self.inView.frame.size.height-self.origin.y-self.frame.size.height)/rightTableView.rowHeight) ? (self.inView.frame.size.height-self.origin.y-self.frame.size.height) : ([rightTableView numberOfRowsInSection:0] * rightTableView.rowHeight);
            }
        }
        CGFloat tableViewHeight = MAX(leftTableViewHeight, rightTableViewHeight);
        [UIView animateWithDuration:0.2 animations:^{
            if (leftTableView) {
                leftTableView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height-0.25, self.frame.size.width*ratio, tableViewHeight);
            }
            if (rightTableView) {
                rightTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height-0.25, self.frame.size.width*(1-ratio), tableViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
...
- (void)animateCollectionView:(UICollectionView *)collectionView show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        if (collectionView) {
            ...
            collectionViewHeight = (([collectionView numberOfItemsInSection:0]+1)/2 > (self.inView.frame.size.height-self.origin.y-self.frame.size.height)/38) ? (self.inView.frame.size.height-self.origin.y-self.frame.size.height) : (([collectionView numberOfItemsInSection:0]+1)/2* 38);
        }
        [UIView animateWithDuration:0.2 animations:^{
            if (collectionView) {
...
- (void)animateTitle:(CATextLayer *)title show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        title.foregroundColor = _leftTableView.tintColor.CGColor;
    } else {
...
#pragma mark 右侧样式
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        cell.textLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:indexPath.row]];
    }
    if (leftOrRight == 1) {
        //右边tableview
