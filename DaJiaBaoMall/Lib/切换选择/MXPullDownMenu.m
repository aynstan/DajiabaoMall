...
- (MXPullDownMenu *)initWithArray:(NSMutableArray *)array selectedColor:(UIColor *)color frame:(CGRect)selfFrame backGroundViewInView:(UIView *)view{
    self = [super init];
    if (self) {
        ...
        if (0<array.count) {
            [self updateContentWithArray:array];
        }
        //创建下边分割线
        UIView *bottomLine=[[UIView alloc]initWithFrame:CGRectMake(self.frame.origin.x, self.frame.size.height-0.5, self.frame.size.width, 0.5)];
        ...
        CGFloat backHeight;
        if (addView==nil) {
            backHeight=[[UIScreen mainScreen] bounds].size.height-(self.frame.origin.y+self.frame.size.height);
        }else{
            backHeight=[[UIScreen mainScreen] bounds].size.height-108-(self.frame.origin.y+self.frame.size.height);
        }
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y+self.frame.size.height, self.frame.size.width,backHeight )];
...
//更新数据
- (void)updateContentWithArray:(NSMutableArray *)array{
    if (array.count==0) {
        return;
    }
    if (0<array.count) {
        ...
        for (CALayer *layer in self.layer.sublayers) {
            if ([layer isKindOfClass:[CATextLayer class]]||[layer isKindOfClass:[CAShapeLayer class]]) {
                dispatch_async(dispatch_get_main_queue(), ^{
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (i != _numOfMenu - 1) {
            ...
            CAShapeLayer *separator = [self creatSeparatorLineWithColor:[UIColor colorWithRed:226.0/255.0 green:226.0/255.0 blue:228.0/255.0 alpha:1.0] andPosition:separatorPosition];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete{
    if (show) {
        ...
        if (addView==nil) {
            addView=[[UIApplication sharedApplication].delegate window];
            tableHeight=[[UIScreen mainScreen] bounds].size.height-(self.frame.origin.y+self.frame.size.height);
        }else{
            tableHeight=[[UIScreen mainScreen] bounds].size.height-108-(self.frame.origin.y+self.frame.size.height);
        }
        [addView addSubview:tableView];

        CGFloat tableViewHeight = ([tableView numberOfRowsInSection:0] > tableHeight/tableView.rowHeight) ? tableHeight: ([tableView numberOfRowsInSection:0] * tableView.rowHeight);
        [UIView animateWithDuration:0.2 animations:^{
...
- (CAShapeLayer *)creatBottomLineWithColor:(UIColor *)color andPosition:(CGPoint)point{
    CAShapeLayer *layer = [CAShapeLayer new];
