...
- (void)savelist:(id)response{
    ...
    if (response) {
        ...
        if (statusCode==0) {
            ...
            NSArray<TixianModel *> *orderArray=[TixianModel mj_objectArrayWithKeyValuesArray:response[@"data"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TixianModel *tixianModel=self.dataSourceArray[indexPath.row];
    if (0<tixianModel.url.length) {
        BaseWebViewController *webView=[[BaseWebViewController alloc]init];
        webView.hidesBottomBarWhenPushed=YES;
        webView.urlStr=tixianModel.url;
...
- (void)addMJheader{
    MJHeader *mjHeader=[MJHeader headerWithRefreshingBlock:^{
        NSString *url=[NSString stringWithFormat:@"%@%@",APPHOSTURL,moneyOut];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"钱包明细-提现支出"];
}
/**
 *  友盟统计页面关闭时间
 *
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"钱包明细-提现支出"];
}
